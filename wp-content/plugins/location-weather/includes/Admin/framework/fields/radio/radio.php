...
if ( ! class_exists( 'SPLWT_Field_radio' ) ) {
	...
	class SPLWT_Field_radio extends SPLWT_Fields {
		...
		public function render() {
			...
			$inline_class = ( $args['inline'] ) ? ' class="lw--inline-list"' : '';
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( is_array( $options ) && ! empty( $options ) ) {
					...
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo ( ! empty( $this->field['empty_message'] ) ) ? esc_attr( $this->field['empty_message'] ) : esc_html__( 'No data provided for this option type.', 'location-weather' );
